...
NGA Reminder 用于 NGA 主题与用户监控、内容保存和通知。服务端正在使用 Go 重构，已完成运行基础、内网管理页、SQLite 和调用链日志；业务功能按阶段继续实现。

## 仓库组成

| 目录 | 状态 | 说明 |
| --- | --- | --- |
| [`service/`](service/docs/README.md) | 阶段 01 已完成 | Go 服务端，使用 Gin、GORM、纯 Go SQLite 和 zerolog，单机运行、禁用 CGO |
...
本地启动和配置说明见 [服务端文档](service/docs/README.md#本地运行)，需要 Go 1.26 或更新版本，构建和自测均使用 `CGO_ENABLED=0`。
