...
NGA Reminder 用于 NGA 主题与用户监控、内容保存和通知。服务端正在使用 Go 重构，已完成运行基础、NGA 账号配置、TID/UID 监控、自动调度与免拉取、手动采集和帖子浏览；其余业务功能按阶段继续实现。

## 仓库组成

| 目录 | 状态 | 说明 |
| --- | --- | --- |
| [`service/`](service/docs/README.md) | 阶段 01～03 已完成 | Go 服务端，使用 Gin、GORM、纯 Go SQLite 和 zerolog，单机运行、禁用 CGO |
