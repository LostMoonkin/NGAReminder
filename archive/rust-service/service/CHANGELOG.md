...
- 新增持久化 UID 历史回帖回填任务，可在管理台为指定监控 UID 选择开始日期，按服务时区静默导入至任务创建时的历史回复；任务支持进度展示、租约恢复和幂等重跑，不推进实时 UID 水位、不创建事件或通知。
- TID 增量抓取现在根据 `floor_number` 断层持久化疑似抽楼，并在首次发现后的 120 分钟内退避复查；审核恢复的楼层即使低于当前高水位也会落库，并继续复用现有事件和通知去重链路。

### 可靠性与质量门

- watch 与资源下载租约增加 claim token fencing，过期执行者不能覆盖重置、暂停或新 worker 的结果。
- Cookie 续期完成改为事务内条件领取，取消、过期或账号不匹配的会话不会写入候选 Cookie。
- 通知坏密文按单条 dead-letter 并记录失败投递，不再终止整个 worker；机器人、系统告警与帖子通知按类别公平消费。
- 修复认证失败监控的恢复标记、资源下载崩溃恢复及 worker 队列饥饿问题。
- NGA 采集与 Cookie 校验不再无限期复用同一条上游连接，避免长时间运行后误判有效 Cookie；登录 HTTP 错误日志现在包含具体状态码。
- CI 新增格式、Rust 全量测试、Clippy、扩展 manifest 与 JavaScript 语法检查，并隔离服务端/Standalone 发布 tag。

### 流式导出、资源维护与内容管理

- Markdown 导出改为稳定复合游标分页和 HTTP 流式响应；ZIP 使用资源目录临时文件逐项写入并在响应结束或中断后删除，不再把完整导出和资源字节保存在内存中。
- 新增资源一致性扫描与显式清理 API/管理页面，支持缺失 ready 文件重排队、孤儿元数据清理，以及超过保留期的孤儿内容文件和导出临时文件清理。
- 新增 UID 监控结果汇总、用户帖子查询和管理台 Markdown/ZIP 导出入口。
- 新增服务端 NGA markup 安全 HTML 渲染，主题和用户详情支持格式、引用、链接、代码及图片，同时拒绝原始 HTML 和不安全 URL。
