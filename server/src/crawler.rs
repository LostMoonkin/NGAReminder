...
impl Crawler {
    ...
    pub async fn fetch_thread_with_page(&self, tid: u64, page: u64, config: &CrawlerConfig) -> Result<NGAThread, Box<dyn Error>> {
        let resp = self.client.post(config.api_url.clone())
            .form(&[("tid", tid), ("page", page)])
            .header(COOKIE, format!("ngaPassportUid={}; ngaPassportCid={}", config.nga_passport_uid, config.nga_passport_cid))
            .send()
            .await?;
        if !resp.status().is_success() {
            println!("Failed to fetch thread with page, Http status not ok {}: {}", tid, resp.status());
        ...
        if optional_code.is_none() {
            println!("Failed to fetch thread with page, invalid response data {}: {}", tid, value);
