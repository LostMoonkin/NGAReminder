...
2026-08-02 已在 dev 环境完成从飞书 owner 交互、验证码提交、候选 Cookie 验证与替换到
认证暂停监控恢复的整体端到端验收。当前 homeserver 单机发布范围不覆盖腾讯验证码、手机安全
验证等额外 challenge；识别到不支持的 challenge 时安全终止并回退管理台人工更新。因此 M8
可以随单一完整 Docker 镜像进入生产发布。
...
- dev 环境整体 Cookie 续期需求已完成端到端验证。
- 生产交付限定为 homeserver 单机 `all` 进程和单一完整 Docker 镜像。
- 手机验证、腾讯验证码等额外 challenge 不属于当前范围，统一安全失败并回退管理台。
