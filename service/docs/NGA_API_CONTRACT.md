...
主题中的普通楼层因审核暂时不可见时，后续可见结果的 `lou`（持久化为 `floor_number`）保留原楼层号并形成断层。非基线 TID 增量采集在新高水位范围内直接根据该断层记录楼层缺口，不使用 `vrows` 推测具体楼层。

缺口在首次发现后的 120 分钟内退避补偿。恢复楼层可以低于当前增量高水位，但只有命中持久化缺口记录时才绕过高水位过滤，并继续使用帖子、事件和通知 outbox 的现有自然键去重。基线中的历史断层不进入补偿。
...
管理员显式创建的 UID 历史回帖回填使用相同的用户回复列表分页和详情作者校验，但使用独立持久任务水位。开始日期按服务配置时区从当天 `00:00:00`（含）转换，结束时间固定为任务创建时刻。任务只保存范围内目标回复、partial 主题元数据和现有 UID 详情语义要求的上下文；它不修改 `user_watch_cursors` 或 `baseline_completed`，也不创建事件、watch match 或通知 outbox。分页结果不再保持 `(postdate, pid)` 倒序时任务必须失败，不能在可能漏帖的情况下提前停止。
