...
- 两种模式都不产生历史通知。后续采集逐页重读，保存新楼层、已记录缺口和新增楼中楼，并按自然键去重；现阶段每轮请求量随主题页数增长。使用首页返回的页数快照，本轮期间新增的尾页留到下轮。
...
到期缺口可在正常采集间隔尚未到达时启动 `gap_recovery` 运行；同一主题同轮只逐页抓一遍，保存已记录缺口及其评论，不移动实时水位或正常计划。正常采集若先看到缺口，也立即恢复。恢复内容按当前作者过滤和渠道生成一次事件/投递，过滤外内容仍保存。最后一次尝试后仍未恢复的缺口标记过期；过期后不主动补抓，后来正常采集看到已记录楼层仍可恢复。重置、修改目标/初始化模式或删除 watch 清除旧缺口，保留正文。
