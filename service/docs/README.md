...
资源容量展示、404 记录、选择性补下载与忽略见 [Spec19](spec/19-resource-retry-management.md)。
...
| `max_download_bytes` | `10485760`（10 MiB），资源下载字节上限，正整数 |
...
`/api/v1/settings` 只返回监听地址、数据路径、时区、后台开关、资源下载上限、raw 保存开关、当前时间和数据库状态；健康检查不返回运行配置。所有应用 HTTP 响应包含 `X-Request-ID`，错误 JSON 中也包含 `trace_id`。路径按表格精确匹配，多余的尾斜杠返回 404。
...
3. 进入详情，可调整星期/时间段覆盖间隔和免拉取时段，也可点击“手动运行一次”。手动请求立即返回，不同 TID/UID 监控可同时运行；页面展示来源、开始/结束时间、页数、新增数量、错误摘要和运行关联 ID。同一 watch 正在运行时重复触发或修改返回 409，其他 watch 可运行和修改；账号操作与资源清理仍需等待采集结束。
4. 从详情或概览的“已保存主题”进入帖子列表。正文按安全富文本显示，支持常用 NGA 标记、来源链接与资源地址；UID watch 详情可进入该用户内容。资源下载和导出见下文。

账号只保留一份。Cookie 以 AES-256-GCM 加密后写入 SQLite；页面/API 只显示脱敏 UID、可用状态和最近校验结果。替换失败保留原凭据及其状态，并展示新凭据失败原因。“校验已保存凭据”确认失效或采集返回认证错误时，账号进入 `auth_paused`；重新校验成功或保存有效新凭据后解除认证暂停，保留用户的暂停选择。失效时独立创建系统告警，向所有启用的 Bark/飞书渠道去重排队；不依赖续期开关。恢复后关闭告警，通知页/API 可查看告警和投递结果，未解除的失败告警可重试。

TID 初始化与重跑规则：

- **保存全部可访问历史（`full`）**：可设置“历史页面并发数”1～16（默认 1），首页之后按配置分批并发抓取，按页序保存主楼、回复和楼中楼，完成整轮后建立静默基线。热门引用不另存一份。
- **从现在开始（`from_now`）**：首次成功运行读取首页和末页，以实际楼层建立水位，不导入旧帖子。后续保存新楼层及实际请求页面上的新评论；旧楼层下的评论按 NGA 发帖时间与基线时间区分，若未提供时间则不把它当作新评论。不为发现历史页的新评论重拉整帖。
- 两种模式都不产生历史通知。后续增量先查首页，回复总数和总页数均未增长、且没有到期缺口时只请求首页；有增长时从水位所在页读到本轮末页，保存新楼层、已记录缺口和符合条件的评论，并按自然键去重。使用首页返回的页数快照，本轮期间新增的尾页留到下轮。
- 每页内容与本轮计数一并提交；整轮成功才提交最终水位、基线和远端分页快照。中途失败保留部分内容但不更新这些进度；重跑通过唯一键复用已存内容，不修改已有正文。旧 Go 数据缺少快照时只从已有水位补查尾部，首次成功后持久化快照，不重建全量基线。
- 手动暂停同时阻止自动和手动运行；恢复后继续使用原水位。主题不存在时标为 `missing`，后续不得自动抓取，可手动重试恢复。待审核标为 `skipped_pending`，保留原水位。
- 修改 TID 或初始化模式会重置基线；只改并发数、备注或调度配置不影响水位。也可选择模式后单独重置基线。重置和删除监控都保留帖子及运行记录，已保存内容始终可从概览进入。

历史页面并发配置见 [Spec11](spec/11-thread-page-concurrency.md)：仅 TID 全量初始化及重置后的全量初始化生效，增量/UID/补偿仍逐页处理。API 字段为 `history_concurrency`，创建时省略默认 1，修改时省略保留原值，SQLite 升级和重启均保留配置。

NGA 单次 HTTP 请求超时 15 秒；所有监控、历史回填和凭据校验共用 120 QPM 限流，相邻请求启动至少间隔 500 毫秒，响应可以重叠，不按监控数量设置并发上限。用户主题/回帖接口返回 2048“服务器忙”时，每次失败响应后等待 3 秒再重试，最多请求 10 次（首次加 9 次重试）；账号校验仍受 25 秒整体超时限制。空 HTTP 503 每两秒重试、最多 3 次。单轮采集失败保留已提交水位；可手动重跑，或等待下一次调度。认证暂停与主题不存在会停止自动抓取，待审核或临时失败则按间隔再次运行。

NGA 采集与凭据校验遇到 302 等重定向时沿用 Rust 自动跟随，最多 10 次跳转，最终响应成功后继续采集；跳转超限或最终响应失败仍保留原水位。每次跳转继续计入 120 QPM；同源保留 Cookie，跨主机、端口或协议时移除敏感头。日志中的 `Following NGA redirect` 可查看跳转状态、次数及来源/目标主机。

UID 初始化与增量规则：

- 每轮先读取用户资料页；成功运行后将 GBK 页面中的用户名写入 API 的 `title` 字段，并显示在监控概览与详情中，不覆盖手工备注。资料页缺少用户对象或返回 UID 不符时标记 `missing` 并停止自动采集，不推进水位；网络或 JSON 解析失败仍可重试。候选主题/PID 不存在只使本轮失败，UID 保持可重试，不能停掉整个用户。
- 首次分别读取主题/回帖列表中首个含有效候选的页面，记录 `(发布时间, TID)` 与 `(发布时间, PID)` 水位，不访问历史帖子详情、不导入历史。不能因无权记录使页面较短而提前结束；无有效候选且还有后页时继续寻找。
- 后续只补全新候选：主题只请求第 1 页并保存目标 UID 的主楼；回帖只按 TID/PID 请求目标内容。详情作者必须仍为目标 UID；按 PID 获取的 `lou=0` 仍保存为回复，不会覆盖主楼或另一个 PID。
- 主题和回帖沿用 Rust 的增量停止规则：处理完整页后，遇到小于或等于原水位的候选即停止翻页，同秒内容按 TID/PID 区分，同页其他新候选仍会保存。不为旧主题的新回复扫描全部历史；没有遇到水位才按分页信息继续。两份列表首页均已达水位、没有新增时，通常共读 3 页（资料、主题、回帖），有新增则按需增加列表页和详情请求。
- 未到水位且未知总数的回复列表满页继续，短页或成功页后的空体 503 结束；已知总数时空体 503 仍是失败。第一页空体 503、业务失败、缺少分页信息均不能当作空列表。
- 两份列表和所需详情全部成功后，内容、两份水位和运行结果在一个 SQLite 事务中提交。失败不写入本轮部分内容，也不推进任意一份水位。明确的成功空列表可以建立空水位。
- 重置或修改 UID 后重新静默建立水位；已有帖子保留并按自然键去重。详情页面显示两份水位，内容可在概览的“已保存主题”浏览。符合条件的增量由阶段 04 通知链路处理；历史回复可通过下文的静默回填补充。

用户初始请求头沿用 Rust：主题/回帖列表仅显式发送配置的 `User-Agent`、`Sec-Fetch-User: ?1` 和三个 Passport Cookie 字段（用户名字段可选）；资料页保留通用头与完整 Cookie，初始 `Referer` 指向目标 UID 的完整资料页 URL，跳转时按上一跳更新。具体约束见 [阶段 03 Spec](spec/03-user-monitoring-and-scheduling.md)。

## 自动调度与免拉取

每个监控的基础间隔默认 60 秒，范围 30～86400 秒；可以添加多个星期/时间段覆盖规则，按配置顺序取第一条命中规则，否则用基础间隔。正常运行结束后以结束时刻的规则计算下次执行时间，并与下一规则开始/结束边界取较早者。例如 08:59 基础间隔一小时、09:00 起每 30 秒，下次执行为 09:00。单进程每秒检查到期监控，按到期顺序启动所有空闲的到期 watch；长任务不会阻塞其他 watch，同一 watch 尚未收尾时不会再次启动。
...
| `POST /api/v1/watches` | TID：`{"kind":"tid","tid":1001,"label":"备注","init_mode":"full","history_concurrency":4}`；UID：`{"kind":"uid","uid":2001,"label":"备注"}`；创建返回 201 |
| `GET /api/v1/watches/:id` | `watch` 和该 watch 最近 20 次 `runs`，以及 `timezone`、`background_enabled` |
| `PUT /api/v1/watches/:id` | 完整目标配置：`kind`、对应的 `tid` 或 `uid`、`label`；TID 还需 `init_mode`，可带 `history_concurrency`（1～16）和调度字段 |
...
基础业务表为 `accounts`、`watches`、`runs`、`posts`，阶段 04～08 增加通知、Bot、续期、资源、历史回填与楼层缺口表；GORM 启动时升级已有 Go 数据库，保留阶段 02 的 TID 水位、内容和暂停状态，并将旧 TID 唯一索引替换为分别约束 TID/UID 的索引。旧会话表不再读写，其他业务表随后续功能增加。帖子通过 `(tid, key)` 唯一键去重：主楼为 `main`，普通回复和楼中楼均为 `pid:<PID>`；楼中楼保留 JSON 嵌套确定的父键，`comment_to_id` 仅作为原始元数据保存。
...
投递异步执行，失败不回滚采集进度；最多尝试五次，重试间隔依次为 1、4、9、16 分钟，耗尽后可手动重试。关闭渠道阻止新增投递和自动重试；被 watch 引用时不能删除，解除引用后删除会保留历史并取消未成功投递。重启保留事件、已读状态与投递结果；外部已接收但本地尚未记成功时可能重发。

飞书卡片按 [Rust 通知规则](spec/04-notifications.md) 渲染原始正文：Reply 标签显示为可读文字，保留粗体、引用、换行等格式；标题超长时截取前 80 字并追加省略号，正文最多 2000 字并带超长提示。仅正文中的前三个图片位置尝试上传，成功图片显示在正文后，失败和超出的图片集中保留为编号链接，最后显示蓝色“查看帖子”按钮。同一应用复用已上传的图片；下载限三个 HTTPS 域名、10 MiB 和 30 秒，允许最多四次白名单内 HTTPS 跳转。Bark 使用原有纯文本摘要。
...
已绑定管理员支持 `/help`、`/status`、`/watch list`、`/watch run <watch_id>`。运行遵循管理页的暂停、账号、单 watch 互斥与 120 QPM 限流，接受后回复运行编号，不代表采集已经成功。群内需显式配置允许的 chat ID，发送者仍须已绑定；绑定仅支持私聊。群地址不回显，替换时勾选“更新允许群”。
...
| `POST /api/v1/renewal/cancel` | 取消活动交互（包括正在进行的网络步骤），无活动时幂等返回 204；原 Cookie 保持不变 |
| `POST /api/v1/renewal/test` | 检查已保存凭据、管理员私聊绑定和 Bot 配置，再探测 NGA 公钥/验证码协议；返回 `ok`、`detail`，不提交密码或发起续期 |
...
资源页的扫描只读，每次打开时统计 SQLite 主库、WAL 与 SHM 的逻辑大小，并统计 assets 中的普通文件总数与逻辑大小；符号链接不计入。页面容量使用 B/KiB/MiB/GiB 可读单位，API 仍返回原始字节整数。可处理的缺失资源默认收起，展开后可全选、反选、补下载所选项或忽略所选项；已确认 404 与已忽略资源只统计数量，不再列出或下载。

页面同时显示无引用元数据、无引用普通文件和临时文件。勾选确认再清理时重新扫描，只删除超过 24 小时且没有正文引用的普通文件和临时文件；保护同内容共享文件，不跟随文件或目录符号链接，不删除正文。资源下载期间清理会等待；有采集或账号操作时返回忙碌提示。
...
| `GET /api/v1/resources` | 只读资源扫描及下载设置；`database_bytes`、`assets_bytes`、`assets_count`、`not_found_count`、`ignored_count` 返回当次统计 |
| `POST /api/v1/resources` | `{"download_enabled":true}`，修改下载开关 |
| `POST /api/v1/resources/redownload` | `{"urls":["https://..."]}`，只重新下载所选可处理缺失项，返回成功数；失败项保留错误并写日志 |
| `POST /api/v1/resources/ignore` | `{"urls":["https://..."]}`，忽略所选可处理缺失项，返回忽略数 |
...
UID watch 详情中填写起始日期，提交历史回帖回填。开始时间为配置时区该日零点（包含），结束时间固定为创建时刻；只读取回复列表及目标 PID 详情，不抓历史主题或整个讨论。详情再次检查 UID 和时间范围，`lou=0` 的 PID 仍作为独立回复保存。后台任务关闭、Cookie 无效、输入错误或已有采集/回填时拒绝提交。

回填与实时监控独立，可在手动暂停、免拉取期间显式运行；不修改水位、基线、手动暂停或调度时间，不生成收件箱事件和投递。导入的帖子为该 UID watch 建立静默观察记录，避免随后重跑把同批历史当作通知。明确的 NGA 认证失败仍会暂停账号并沿用续期提示，手动暂停配置不变。

每个详情的正文与任务进度一起提交；失败保留已提交内容和计数，可重新提交相同或重叠日期范围。一个进程同时只执行一次采集或回填；管理页展示最近 20 次回填的时间范围、状态、请求页数（含详情）、候选数、保存数、错误与关联 ID。正常停机取消任务，启动时将异常遗留的 `running` 标记 `interrupted`，不自动断点续传。

TID 完成基线后，成功增量采集中从旧水位到本轮最高实际楼层之间的断层建立缺口；只看主楼/普通楼层号，不根据总回复数或评论推测。基线旧断层不补。缺口与最终水位一同提交，已保存内容按页提交的规则保持；失败的整轮不建立新缺口或推进水位。

缺口从首次发现起持续最多 120 分钟，在第 5、15、35、65、95、120 分钟附近合并尝试；最后一轮预留最多一个一秒调度 tick。暂停、免拉取和账号无效时不自动访问 NGA，期限继续流逝。延迟或重启只执行最近到期的一轮，不集中补跑错过的次数，首次发现时间和截止时间不重新计算。

到期缺口可在正常采集间隔尚未到达时启动 `gap_recovery` 运行；先查首页确定分页，只抓到期缺口的提示页及前后各一页，同轮与增量页面去重，不遍历整个主题。新缺口保存发现时附近可见楼层的页码，旧记录没有提示页时按楼层与每页条数估算。独立补偿只保存已记录缺口及其评论，不移动实时水位、分页快照或正常计划。正常采集若先看到缺口，也立即恢复。恢复内容按当前作者过滤和渠道生成一次事件/投递，过滤外内容仍保存。最后一次尝试后仍未恢复的缺口标记过期；过期后不主动补抓，后来正常采集看到已记录楼层仍可恢复。重置、修改目标/初始化模式或删除 watch 清除旧缺口，保留正文。

| API（Bearer token） | 参数 / 行为 |
| --- | --- |
| `POST /api/v1/watches/:id/backfills` | `{"start_date":"2026-01-01"}`；仅 UID watch，返回 202、任务及状态查询 Location |
| `GET /api/v1/backfills/:id` | 查询回填范围、状态、进度、错误及关联 ID |
| `GET /api/v1/watches/:id` | 增加最近 `backfills` 和缺口 `gaps` 摘要（`pending`、`resolved`、`expired`） |

本阶段不执行旧 PG 迁移或正式上线；下一步按 [阶段 09 数据迁移](spec/09-data-migration.md) 准备离线工具与演练，再进入阶段 10。
