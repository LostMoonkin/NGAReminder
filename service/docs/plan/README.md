...
阶段 01～09 已完成实现与自测；阶段 09 提供完整 PG 导出与 SQLite 转换脚本，实际旧库迁移留待部署时执行。阶段 10 已提供容器部署模板与 GHCR 镜像发布工作流，正式上线验收待完成。这里统一维护新服务端的阶段安排与进度，具体需求和验收见相应 Spec；[历史项目计划](../../../archive/rust-service/PROJECT_PLAN.md) 保留在归档中。
...
| 05 | [飞书机器人](../spec/05-feishu-bot.md) | 绑定管理员，在飞书查询状态、查看监控和手动运行 | 04 | 已完成 |
| 06 | [Cookie 续期](../spec/06-cookie-renewal.md) | 在飞书确认登录、提交验证码并恢复因认证暂停的监控 | 05 | 已完成 |
| 07 | [内容浏览、资源与导出](../spec/07-content-and-export.md) | 浏览富文本，导出 Markdown/ZIP，维护本地资源 | 03 | 已完成 |
| 08 | [历史回填与楼层补偿](../spec/08-history-and-gap-recovery.md) | 回填指定日期后的用户回帖，补收审核后恢复的楼层 | 04、07 | 已完成 |
| 09 | [数据迁移](../spec/09-data-migration.md) | 完整导出 PG 并转换为 Go SQLite 文件，保留原 assets 挂载；见[操作说明](migration-runbook.md) | 01～08 | 工具与自测已完成 |
| 10 | [单机交付与上线](../spec/10-single-host-delivery.md) | 使用迁入数据启用 Go，并按中文说明部署、备份和恢复 | 01～09 | 部分完成：容器部署与 GHCR 镜像发布 |
...
界面重设计：[Spec15：B 方案管理台](../spec/15-admin-ui.md)，已完成实现与自测。按用户选定的顶部导航与状态看板实现，统一监控、内容、收件箱、通知渠道和设置入口。

补充功能：[Spec16：按时段导出内容](../spec/16-time-ranged-content-export.md)，已完成实现与自测。为 TID/UID 的 Markdown 与 ZIP 导出增加可选秒级闭区间，并将内容页导出入口整合为范围与格式选择悬浮窗。

工程优化：[Spec17：服务端全量测试提速](../spec/17-test-runtime.md)，已完成实现与自测。主 Go module 禁用测试结果缓存后的全量测试保持在 30 秒内，生产 NGA 限流、重试和后台周期不变。

补充功能：[Spec18：资源磁盘占用统计](../spec/18-resource-storage-usage.md)，已完成实现与自测。资源维护页按请求统计 SQLite 的当前逻辑大小，以及 assets 的普通文件总数和逻辑大小。

补充功能：[Spec19：资源维护展示与缺失资源管理](../spec/19-resource-retry-management.md)，已完成实现与自测。资源页使用可读容量单位，持久化记录 404 与忽略状态，支持选择性补下载和忽略。
...
- 新服务使用独立的 SQLite 文件，assets 继续挂载原目录。阶段 09 负责旧 Rust PG 数据的完整导出与离线转换，保留源库和备份，不复制或改动资源文件；旧 API 逐字段兼容仍不在范围内，不能覆盖、清空或原地升级旧库。
