...
- TID/UID 共用自动调度、运行入口和结果展示；抓取及游标语义各自保持明确。不同 watch 的并行运行及账号限流按后续 [Spec11](11-thread-page-concurrency.md) 执行。
...
- [x] 重复触发不会并发采集同一 watch；重启后的中断运行可见且可以重新执行。
- [x] 修复账号后认证暂停恢复、手动暂停保持，运行和后台请求日志均可关联。

## 实现与自测

已完成（2026-09-17）。管理页与 API 用法见 [服务端文档](../README.md#自动调度与免拉取)。

- 使用脱敏 NGA fixture、真实 service/Gin 和临时 SQLite 验证 UID 静默基线、详情作者核对、同秒不同 PID 与 `lou=0`、失败不提交水位、重置/删除保留内容，以及认证恢复保留手动暂停。
- 固定时间验证首条间隔覆盖、星期、跨午夜、首尾边界、连续免拉取合并和每周全天免拉取；运行入口验证自动跳过无 NGA 请求、手动绕过、暂停限制和串行互斥。
