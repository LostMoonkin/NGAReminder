...
func newFloorGaps(watch repository.Watch, seen map[int64]bool, maxFloor int64, now time.Time) []repository.FloorGap {
	result := []repository.FloorGap{}
	if !watch.BaselineComplete {
		return result
	}
	for floor := watch.CursorFloor; floor < maxFloor; {
		floor++
		if !seen[floor] {
			result = append(result, repository.FloorGap{WatchID: watch.ID, Floor: floor, Status: "pending", FirstSeen: now.UTC(), Deadline: now.Add(120 * time.Minute).UTC()})
		}
	}
	return result
}
...
func advanceGapAttempts(ctx context.Context, tx *repository.Store, watchID int64, now time.Time) error {
